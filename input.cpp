...
HRESULT InitializeMouse(HWND hWindow); // マウスの初期化
...
//=============================================================================
// マウスの初期化
HRESULT InitializeMouse(HWND hWindow)
{
	HRESULT result;
	// デバイス作成
...
THUMB_DATA GetThumbLeft(int padNo)
{
	THUMB_DATA left_data = { g_Controllers[padNo].state.Gamepad.sThumbLX ,
		g_Controllers[padNo].state.Gamepad.sThumbLY };
	return left_data;
}
...
THUMB_DATA GetThumbRight(int padNo)
{
	THUMB_DATA right_data = { g_Controllers[padNo].state.Gamepad.sThumbRX ,
		g_Controllers[padNo].state.Gamepad.sThumbRY };
	return right_data;
}
...
{
	speed %= 256;
	g_Controllers[padNo].vibration.wLeftMotorSpeed = (WORD)((speed + 1) * 256) - 1;
}

void SetRightVibration(int padNo, int speed)
{
	speed %= 256;
	g_Controllers[padNo].vibration.wRightMotorSpeed = (WORD)((speed + 1) * 256) - 1;
}
